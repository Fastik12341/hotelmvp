    ...
    <div className="min-h-screen bg-[#F5F2ED]" style={{ fontFamily: "'Cormorant Garamond', 'Georgia', serif" }}>
      ...
      <header className="bg-white border-b border-[#E5E0D8]">
        <div className="max-w-7xl mx-auto px-6 py-5 flex items-center justify-between">
          <div>
            <h1 className="text-xl font-normal text-[#2E2E2E]">Admin Panel</h1>
            <p className="text-xs text-[#B0A89A]">Mövenpick Siam Pattaya</p>
          </div>
              ...
              >
                <div className="flex flex-wrap items-start justify-between gap-4 mb-4">
                  <div className="flex items-center gap-3">
                    <span className="text-xl font-normal text-[#2E2E2E] tracking-wider">
                      {request.code}
                    </span>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                  <div>
                    <span className="text-xs text-[#B0A89A] uppercase">Гость</span>
                    <p className="text-[#2E2E2E] mt-1">{request.guestName}</p>
                  </div>
                  <div>
                    <span className="text-xs text-[#B0A89A] uppercase">Комната</span>
                    <p className="text-[#2E2E2E] mt-1">{request.roomNumber}</p>
                  </div>
                  <div>
                    <span className="text-xs text-[#B0A89A] uppercase">Услуга</span>
                    <p className="text-[#2E2E2E] mt-1">{request.service}</p>
                  </div>
                  <div>
                    <span className="text-xs text-[#B0A89A] uppercase">Дата / Время</span>
                    <p className="text-[#2E2E2E] mt-1">{request.date} / {request.time}</p>
                  </div>
                  <div>
                    <span className="text-xs text-[#B0A89A] uppercase">Гостей</span>
                    <p className="text-[#2E2E2E] mt-1">{request.guests}</p>
                  </div>
                  {request.comment && (
                ...
                <div className="flex flex-wrap items-center gap-2 pt-4 border-t border-[#E5E0D8]">
                  <span className="text-xs text-[#B0A89A] mr-2">Статус:</span>
                    ...
                    <button
                      ...
                      className={`px-3 py-1 text-xs transition-colors ${
                        request.status === status
                          ? 'bg-[#8C7343] text-white'
                          : 'bg-[#F5F2ED] text-[#6B6B6B] hover:bg-[#E5E0D8]'
                      }`}
                    >
                      {statusLabels[status]}
                    </button>
